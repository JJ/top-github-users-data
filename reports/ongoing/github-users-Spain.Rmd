...
Except for Valencia, Barcelona and Madrid, the rest of the provinces are not the most populated in Spain. That is why, if we take into account the population, dividing the number of GitHub users by the provincial population (as published by the National Statistics Institute), the situation is somewhat different, with Barcelona, Madrid and Granada emerging as the province with the highest number of GitHub active developers per capita; they are head-to-head and the situation might change from one report to the next; that is, a clear winner is not declared.

>In this the fact that GitHub search returns different numbers in two consecutive requests might play a role. There is always some degree of uncertainty.

After them the situation is more stable, with Madrid in the third position, followed by Seville and Saragossa (Zaragoza), Gipuzkoa (represented by its main city, Bilbao), Valencia and Valladolid. The province of Córdoba is a distant next, with Coruña coming up as the last of the top 10.
...
The situation is similar if we take into account the aggregated contributions by all users that have declared their province. Once again, Granada emerges as the winner, with Navarra, largely due to a single user, as the second. Next, the positions of Barcelona and Madrid is inverted and a new player, Valladolid, enters the top five. The first 6 positions seem consolidated (and closed by Zaragoza), but Bilbao and Sevilla are very close in the 7th and 8th, and Valencia and Gipuzkoa (represented by its capital city, Donostya), vying for the next positions up to the tenth.
