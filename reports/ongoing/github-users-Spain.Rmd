---
title: "GitHub users in Spain; May 31st 2015"
author: "JJ Merelo"
date: "31/5/2015"
...
Except for Valencia, Barcelona and Madrid, the rest of the provinces are not the most populated in Spain. That is why, if we take into account the population, dividing the number of GitHub users by the provincial population (as published by the National Statistics Institute), the situation is somewhat different, with Barcelona emerging as the province with the highest number of GitHub active developers per capita, followed by Granada and Madrid and then by Seville and Saragossa.

```{r, echo=FALSE,fig.width=10,fig.height=8}
ggplot(data.province, aes(x=reorder(province,usersrel),y=usersrel))+geom_bar(stat='identity')+coord_flip()
```

The situation is similar if we take into account the aggregated contributions by all province users. Once again, Granada emerges as the winner but the position of Barcelona and Madrid is inverted and a new player, Valladolid, enters the top five. Navarra, due to a single user with more than 20000 contributions, has managed to enter that ranking too.

```{r, echo=FALSE,fig.width=10,fig.height=8}
ggplot(data.province, aes(x=reorder(province,contribrel),y=contribrel))+geom_bar(stat='identity')+coord_flip()
```

The aggregated number of followers is a bit less surprising, with Madrid and Barcelona on top, but Álava and Bilbao entering the top 5. If we delve into the data this is mainly due to a single user in both cases. 
...
Finally, if we consider the number of stars given by users, a new one, Tenerife, gets into the top 10.
```{r, echo=FALSE,fig.width=10,fig.height=8}
...
Apparently, one cluster includes the provinces with the highest productivity and then two provinces which excel in the number of stars given to projects participated by developers there: Madrid, Barcelona, Zaragoza and Granada. A second cluster, in turquoise, includes Sevilla, Navarra, Bilbao and Álava, which some times score in the top 10 in every ranking and in some cases the top 5. The following cluster includes  provinces like Salamanca or Pontevedra with some scores in the top, and vinally the red cluster which does not have anything special to show.

This clustering varies each week, however, although Barcelona, Madrid and Granada are almost always placed in the top cluster, and most provinces grouped in a single cluster. Clusters vary between 3 and 4, depending on the week.
